...
Z = cor(CorrAvg[2:9])
View(Z)
corrplot(Z,method = "circle", addCoef.col = "black" ,type="upper", order="hclust", tl.col= "black", tl.srt= 45, col=brewer.pal(n=8, name="YlGnBu"))
...
ggplot(data=maxthread, aes(x=maxthread$Time,y=maxthread$WC)) + 
  labs(title="Amount of Word Count v Time of Day (Most Active Thread: ID:283958)", x="Time (hours)", y="Word Count ") +
...
r	=	r	+	scale_fill_manual(breaks =	c("Analytic",	 "Clout",  "Authentic",	  "Tone",	"OtherLIWC"), values = c("#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7"))
...
library(plyr)
cleanWF = webforum[!(webforum$WC==0),]
ThreadGroup = aggregate(cleanWF[6:29], cleanWF[1], mean)
...
top40Info=subset(cleanWF,ThreadID%in%Top40Threads$ThreadID)
...
ggplot(Amelt, aes(year, value, fill=variable)) +
  ...
  scale_fill_manual(values = alpha(c("#00AFBB",  "#E7B800",   "#FC4E07", "#293352", "#52854C"), .25)) +
  labs(
